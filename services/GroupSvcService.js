...
const GroupSvcService = {
    ...
    async findAllServiceHealth(services) {
        let results = []
        for (let i = 0; i < services.length; i++) {
            try {
                ...
                const result = this.findOneServiceHealth(service)
    ...
    async findOneServiceHealth(service) {
        let result = {}
        try {
            if (!service["serverId"] || service["serverId"] === '-1') {
                return result
            }

            const serverInfo = await ServerService.findServerById(service["serverId"])

            if (service["type"] === 'container') {
                const serviceNames = await FileUtil.getServiceNameList({groupId: service['groupId'], serviceId: service["id"]})

                const servicePath = await FileUtil.getDockerComposeServicePath({groupId: service['groupId'], serviceId: service["id"]})
