...
function Services() {
    ...
    const handleAddService = () => {
        ...
        fetch(`/api/groups/${groupId}/services`, {
            ...
            body: JSON.stringify({
                name, server, type,
                variables: variables.filter(variable => variable['key'] && variable['val']),
                yaml,
                pidCmd, startScript, stopScript,
                logFiles: logFiles.filter(logFile => logFile['key'] && logFile['val']),
            })
        })
            .then(res => res.json())
            .then(body => {
                            ...
                            <List dense={true}>

                                {
                                    variables.map((variable, index) => {
                                        return (
                                            <ListItem key={variable['key'] + "-" + index}>
                                                <Grid container>
                                                    <Grid item xs={1}>
                                                    ...
                                                    <Grid item xs={4}>
                                                        <TextField fullWidth
                                                                   ...
                                                                   required={true}
                                                                   value={variable['key']}
                                                                   onChange={event => variable['key'] = event.target.value}
                                                        />
                                                    </Grid>
                                                    <Grid item xs={1}>
                                                    ...
                                                    <Grid item xs={4}>
                                                        <TextField fullWidth
                                                                   ...
                                                                   required={true}
                                                                   value={variable['val']}
                                                                   onChange={event => variable['val'] = event.target.value}
                                                        />
                                                    </Grid>
                                                </Grid>
